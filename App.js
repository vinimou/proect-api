...
export default function App() {
  const { get, response, loading, error, post, del, put } = useFetch(baseURL);
  ...
  const buscarPosts = async (user) => {
    setSelectedUser(user);
    const data = await get("/posts?userId=" + user._id);
    if (response.ok) {
      setPosts(data);
    } else {
      setPosts([]);
  ...
  if (error) return `Erro: SE FUDEU AMIGAO ${error.message}`;
